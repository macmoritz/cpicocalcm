...
org	100h ; setting origin to 100h for compatibility with tnylpo
tran	equ	100h
tranm	equ	$
ccploc	equ	$
;
...
;	utility procedures
printchar:
	mov e,a! mvi c,pcharf! call bdos! ret
;
printbc:
...
;
initialize:
	mvi c,initf! call bdos! ret
;
select:
	mov e,a! mvi c,self! call bdos! ret
;
bdos$inr:
...
;
setuser:
        mvi c,userf! call bdos! ret ;sets user number
;
saveuser:
	...
	user0:	;file name is present
		...
		bmove3:	;b has character count
			...
			call crlf
                        ...
                        lxi h, comfcb ; copy from
                        lxi d, fcb    ; copy to
                        mvi b, 33     ; copy 33 bytes
                        call move0
